...
void MotionModel::newOdometry(const OdometryReadingInterface *o){
  ...
  if(t_curr < 0.0){
	...
	Dir_bias = -deg2rad(0.46)*Vt;
      ...
      double stt = POW2(0.4*dt/3);
      double saa = POW2(0.1*rot/3) + POW2(dt*0.5*DEGTORAD/3);
      double srr = POW2(0.1*r/3)   + POW2(dt*0.00001/3);

      cov[2][2] = POW2(0.5*DEGTORAD/3) + POW2(0.1*rot/3);

...
The particle filter is introduced in Section~\ref{sec:PF}. A brief
overview of Localisation is given in Section~\ref{sec:Localisation},
followed by an overview of SLAM in Section~\ref{sec:SLAM}. Finally the
...
observable discrete time Markov chain\cite{fixme}. The state of the
...
To illustrate this point lets consider an atypically simple mapping
problem: the world consists of two point landmarks and a robot living
on a two-dimensional plane, see \refFigure{fig:post_all}. The robot
...
then as shown in \refFigure{fig:post_obsodo_}. As one can see this

...
By limiting the area of the local map, the HTSLAM algorithm achieves two major goals:
firstly the computation requirements per local map are bounded, since there is a finite number of landmarks in a finite area, secondly the residual uncertainty of the robot pose within the local region is bounded.

TODO: accentuate the difference in approach from Atlas, that constantly monitors the pose uncertainty/performance of the mapper/size of the map.

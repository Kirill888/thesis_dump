...
very obvious how to model error propagation. In this work errors in the
...
H=0.76m - half the distance between the rear wheels. Laser sensor is
mounted at 3.78m, 0.5m.
...
possible to assign any region to the local map. A grid map with the
resolution of 50cm is used. Initial map region is a rectangle aligned
with robot heading direction. At the end of the mapping the region is
trimmed, refer to \refFigure{fig:initial_region_vision}, it is set to
the intersection of the initial region and the region formed by the
convex hull of the map.
...
The HTSLAM algorithm ran 100 times on each data set. The map from each
run was classified as one of: consistent and proper (closed loop),
consistent but improper (failed to close the loop) or inconsistent.
